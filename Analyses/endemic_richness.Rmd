...
write.table(AliveBefore, "BDS/AliveBefore.txt",
...
write.table(PyRateInput, 
            file = paste0("BDS/TrimBounded.txt"), 
            sep = "\t", row.names = FALSE) 
extract.ages(file = paste0("BDS/TrimBounded.txt"), replicates = 2, outname = "")
```
...
write.table(PresShifts, 
            file = paste0("BDS/PresShifts.txt"), 
...
runPyRate <- function(i){
     system( paste("python2 PyRate/PyRate.py",
                 "BDS/TrimBounded.py",
                  ...
                  "-mL 1",
                  "-mM 1",
                  "-trait_file BDS/AliveBefore.txt", # see Pires et al. 2018
                  "-bound 13.66 0", 
                  "-qShift BDS/PresShifts.txt", # Preservation shifts
                  "-wd BDS")) 
}
...
```{r}
McmcLogFiles <- list.files("BDS/pyrate_mcmc_logs", pattern = "GBD1-1_mcmc.log")
McmcLogs <- vector(mode = "list", length(McmcLogFiles)) 
for(i in 1:length(McmcLogFiles)){
  McmcLogs[[i]] <- read.table(paste0("BDS/pyrate_mcmc_logs/", McmcLogFiles[i]), 
                              header = TRUE, sep = "\t")
}
